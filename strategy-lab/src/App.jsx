...
import { XAxis, YAxis, Tooltip, ResponsiveContainer, ReferenceLine, Area, AreaChart, ComposedChart, Bar, CartesianGrid, Line } from "recharts";
...
const MO=`'JetBrains Mono','Fira Code',monospace`;const SA=`'DM Sans',sans-serif`;
const C={bg:"#06090f",sf:"#0c1119",s2:"#131c2b",s3:"#1a2540",bd:"#1a2236",tx:"#e2e8f0",tm:"#94a3b8",td:"#5a6b85",g:"#10b981",gd:"#10b98118",r:"#f43f5e",rd:"#f43f5e18",b:"#3b82f6",bd2:"#3b82f618",am:"#f59e0b",cy:"#06b6d4",pu:"#a855f7",vi:"#7c3aed",ac:"#3b82f6"};
...
function btCode(data,entryCode,exitCode,cfg){
  ...
  let entryFn,exitFn;
  try{entryFn=new Function("ctx","with(ctx){return("+entryCode+")}");exitFn=new Function("ctx","with(ctx){return("+exitCode+")}")}catch(e){return{trades:[],eq:[],stats:{ret:0,fin:c0,n:0,wr:0,aw:0,al:0,pf:0,mdd:0,sh:0,exp:0,ah:0,wn:0,ls:0},error:e.message}}
...
// ── UI ATOMS ──
const Btn=({children,active,color,onClick,style:sx})=><button onClick={onClick} style={{background:active?(color||C.ac):"transparent",color:active?"#fff":(color||C.tm),border:`1px solid ${active?(color||C.ac):C.bd}`,borderRadius:6,padding:"6px 14px",fontSize:11,fontFamily:MO,cursor:"pointer",transition:"all .15s",whiteSpace:"nowrap",...sx}}>{children}</button>;
const Stat=({label,value,sub,color})=><div style={{background:C.sf,border:`1px solid ${C.bd}`,borderRadius:8,padding:"10px 14px"}}><div style={{fontFamily:SA,fontSize:10,color:C.td,textTransform:"uppercase",letterSpacing:".08em",marginBottom:3}}>{label}</div><div style={{fontFamily:MO,fontSize:18,fontWeight:700,color:color||C.tx,lineHeight:1.2}}>{value}</div>{sub&&<div style={{fontFamily:MO,fontSize:10,color:C.td,marginTop:2}}>{sub}</div>}</div>;
const Sel=({value,onChange,options,style:sx})=><select value={value} onChange={e=>onChange(e.target.value)} style={{background:C.s2,color:C.tx,border:`1px solid ${C.bd}`,borderRadius:5,padding:"5px 8px",fontFamily:MO,fontSize:11,outline:"none",cursor:"pointer",...sx}}>{options.map(o=><option key={o.v} value={o.v}>{o.l}</option>)}</select>;
const Num=({value,onChange,min,max,step,style:sx})=><input type="number" value={value} min={min} max={max} step={step||1} onChange={e=>onChange(+e.target.value)} style={{background:C.s2,color:C.tx,border:`1px solid ${C.bd}`,borderRadius:5,padding:"5px 8px",fontFamily:MO,fontSize:11,width:65,outline:"none",...sx}}/>;
...
const CR=({c,onChange,onRm})=><div style={{display:"flex",alignItems:"center",gap:6,padding:"6px 0",borderBottom:`1px solid ${C.bd}`,flexWrap:"wrap"}}><Sel value={c.i} onChange={v=>onChange({...c,i:v})} options={IO}/>{!["price"].includes(c.i)&&<Num value={c.p1} onChange={v=>onChange({...c,p1:v})} min={1} max={500}/>}<Sel value={c.c} onChange={v=>onChange({...c,c:v})} options={CO}/><Sel value={c.t} onChange={v=>onChange({...c,t:v})} options={TO}/><Num value={c.p2} onChange={v=>onChange({...c,p2:v})} min={0} max={500}/><button onClick={onRm} style={{background:C.rd,color:C.r,border:"none",borderRadius:4,padding:"3px 8px",cursor:"pointer",fontFamily:MO,fontSize:11}}>×</button></div>;
const TT=({active,payload,label})=>{if(!active||!payload?.length)return null;return<div style={{background:C.s2,border:`1px solid ${C.bd}`,borderRadius:6,padding:"8px 12px",fontFamily:MO,fontSize:10}}><div style={{color:C.tm,marginBottom:4}}>{label}</div>{payload.filter(p=>p.value!=null).map((p,i)=><div key={i} style={{color:p.color||C.tx}}>{p.name}: {typeof p.value==="number"?p.value.toLocaleString(undefined,{maximumFractionDigits:2}):p.value}</div>)}</div>};
...
export default function StrategyLab(){
  ...
  return <div style={{background:C.bg,minHeight:"100vh",color:C.tx,fontFamily:SA,display:"flex",flexDirection:"column"}}>
    ...
    <style>{`*{box-sizing:border-box;scrollbar-width:thin;scrollbar-color:${C.s3} transparent}::-webkit-scrollbar{width:5px}::-webkit-scrollbar-thumb{background:${C.s3};border-radius:3px}::selection{background:${C.ac}40}@keyframes fi{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}@keyframes pu{0%,100%{opacity:.4}50%{opacity:1}}.fi{animation:fi .3s ease both}textarea,input[type=text],input[type=password],input[type=number]{font-size:16px}@media(min-width:600px){textarea,input[type=text],input[type=password],input[type=number]{font-size:11px}}`}</style>

    {/* HEADER */}
    <div style={{background:C.sf,borderBottom:`1px solid ${C.bd}`,padding:"0 16px",display:"flex",alignItems:"center",height:52,gap:12,flexShrink:0,flexWrap:"wrap"}}>
      <img src={LOGO} alt="Strategy Lab" style={{width:36,height:36,borderRadius:8}}/>
      <div><div style={{fontSize:15,fontWeight:700,letterSpacing:"-.02em",lineHeight:1.1}}>Strategy <span style={{color:C.g}}>Lab</span></div><div style={{fontSize:8,color:C.td,fontFamily:MO,letterSpacing:".06em"}}>BACKTEST · SIMULATE · OPTIMIZE</div></div>
      <div style={{width:1,height:24,background:C.bd}}/>
      {/* Asset Picker */}
      <select value={asset} onChange={e=>{setAsset(e.target.value)}} style={{background:C.s2,color:C.tx,border:`1px solid ${C.bd}`,borderRadius:6,padding:"5px 10px",fontFamily:MO,fontSize:12,fontWeight:700,outline:"none",cursor:"pointer",maxWidth:140}}>
        {ASSETS.map(a=><option key={a.id} value={a.id}>{a.sym} — {a.name}</option>)}
      </select>
      <select value={tfDays} onChange={e=>setTfDays(+e.target.value)} style={{background:C.s2,color:C.tx,border:`1px solid ${C.bd}`,borderRadius:6,padding:"5px 8px",fontFamily:MO,fontSize:11,outline:"none",cursor:"pointer"}}>
        {TF_DAYS.map(t=><option key={t.v} value={t.v}>{t.l}</option>)}
      </select>
      {dataLoading?<span style={{fontFamily:MO,fontSize:10,color:C.am}}>Loading...</span>
      :<span style={{fontFamily:MO,fontSize:9,color:dataSource==="live"?C.g:C.td,background:dataSource==="live"?C.gd:"transparent",padding:"2px 6px",borderRadius:3}}>{dataSource==="live"?"● LIVE DATA":"○ SYNTHETIC"}</span>}
      <button onClick={()=>loadData(asset,tfDays)} title="Refresh data" style={{background:"transparent",border:`1px solid ${C.bd}`,borderRadius:5,padding:"4px 8px",color:C.tm,fontFamily:MO,fontSize:11,cursor:"pointer"}}>↻</button>
      <div style={{flex:1}}/>
      <div style={{display:"flex",gap:2,flexWrap:"wrap"}}>{NAV.map(n=><Btn key={n.id} active={pg===n.id} onClick={()=>setPg(n.id)} style={{padding:"5px 10px",fontSize:10}}><span style={{marginRight:3}}>{n.icon}</span>{n.l}</Btn>)}</div>
      <div style={{flex:1}}/>
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        <input ref={importRef} type="file" accept=".json" onChange={importData} style={{display:"none"}}/>
        <button onClick={()=>importRef.current?.click()} title="Import backup" style={{background:"transparent",border:`1px solid ${C.bd}`,borderRadius:5,padding:"4px 8px",color:C.td,fontFamily:MO,fontSize:9,cursor:"pointer"}}>↑ Import</button>
        <button onClick={exportData} title="Export all data" style={{background:"transparent",border:`1px solid ${C.bd}`,borderRadius:5,padding:"4px 8px",color:C.td,fontFamily:MO,fontSize:9,cursor:"pointer"}}>↓ Export</button>
        <div style={{width:1,height:16,background:C.bd}}/>
        <div style={{width:6,height:6,borderRadius:"50%",background:rc}}/><span style={{fontFamily:MO,fontSize:12,color:rc,fontWeight:600}}>{s.ret>0?"+":""}{s.ret}%</span></div>
    </div>

    <div style={{display:"flex",flex:1,overflow:"hidden"}}>
      {/* SIDEBAR */}
      <div style={{width:sideO?320:42,minWidth:sideO?320:42,background:C.sf,borderRight:`1px solid ${C.bd}`,transition:"width .2s",overflow:"hidden",display:"flex",flexDirection:"column"}}>
        <button onClick={()=>setSideO(!sideO)} style={{background:"transparent",border:"none",color:C.tm,padding:"8px",cursor:"pointer",fontFamily:MO,fontSize:13,textAlign:"center",borderBottom:`1px solid ${C.bd}`}}>{sideO?"◂":"▸"}</button>
        {sideO&&<div style={{padding:14,overflowY:"auto",flex:1}} className="fi">
          {/* Mode Toggle */}
          <div style={{display:"flex",gap:3,marginBottom:14,background:C.s2,borderRadius:6,padding:3}}>
            <button onClick={()=>setMode("visual")} style={{flex:1,padding:"6px 0",borderRadius:4,border:"none",background:mode==="visual"?C.ac:"transparent",color:mode==="visual"?"#fff":C.tm,fontFamily:MO,fontSize:10,cursor:"pointer",fontWeight:600}}>◧ Visual</button>
            <button onClick={()=>setMode("code")} style={{flex:1,padding:"6px 0",borderRadius:4,border:"none",background:mode==="code"?C.vi:"transparent",color:mode==="code"?"#fff":C.tm,fontFamily:MO,fontSize:10,cursor:"pointer",fontWeight:600}}>⟨/⟩ Code</button>
          </div>

          {mode==="code"?<>
            {/* Code Presets */}
            <div style={{marginBottom:12}}><div style={{fontSize:9,fontWeight:600,color:C.td,textTransform:"uppercase",letterSpacing:".1em",marginBottom:6}}>Code Templates</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:3}}>{Object.entries(CODE_PRESETS).map(([k,v])=><button key={k} onClick={()=>{setCodeEn(v.en);setCodeEx(v.ex);setCodeErr("")}} style={{background:C.s2,border:`1px solid ${C.bd}`,borderRadius:5,padding:"4px 8px",fontSize:8,fontFamily:MO,color:C.tm,cursor:"pointer"}}>{v.n}</button>)}</div></div>

            {/* Entry Code */}
            <div style={{marginBottom:12}}>
              <div style={{fontSize:9,fontWeight:600,color:C.g,textTransform:"uppercase",letterSpacing:".1em",marginBottom:4}}>▲ Entry Condition</div>
              <textarea value={codeEn} onChange={e=>{setCodeEn(e.target.value);setCodeErr("")}} spellCheck={false} style={{width:"100%",height:110,background:"#0a0f18",color:C.g,border:`1px solid ${codeErr?C.r:C.bd}`,borderRadius:6,padding:8,fontFamily:MO,fontSize:10,outline:"none",resize:"vertical",lineHeight:1.6,tabSize:2}}/>
            </div>

            {/* Exit Code */}
            <div style={{marginBottom:12}}>
              <div style={{fontSize:9,fontWeight:600,color:C.r,textTransform:"uppercase",letterSpacing:".1em",marginBottom:4}}>▼ Exit Condition</div>
              <textarea value={codeEx} onChange={e=>{setCodeEx(e.target.value);setCodeErr("")}} spellCheck={false} style={{width:"100%",height:80,background:"#0a0f18",color:C.r,border:`1px solid ${codeErr?C.r:C.bd}`,borderRadius:6,padding:8,fontFamily:MO,fontSize:10,outline:"none",resize:"vertical",lineHeight:1.6,tabSize:2}}/>
            </div>

            {codeErr&&<div style={{background:C.rd,border:`1px solid ${C.r}30`,borderRadius:6,padding:8,fontFamily:MO,fontSize:9,color:C.r,marginBottom:10,lineHeight:1.5}}>⚠ {codeErr}</div>}

            {/* Syntax Reference */}
            <div style={{background:"#0a0f18",borderRadius:6,padding:10,fontFamily:MO,fontSize:8,color:C.td,lineHeight:1.8,marginBottom:12}}>
              <div style={{color:C.cy,fontSize:9,fontWeight:600,marginBottom:4}}>SYNTAX REFERENCE</div>
              <div style={{color:C.g}}>Indicators:</div>
              <div>SMA(20) EMA(9) RSI(14)</div>
              <div>MACD_HIST() MACD_LINE()</div>
              <div>BB_UPPER(20) BB_LOWER(20) BB_MID(20)</div>
              <div>ATR(14) STOCH_K(14) STOCH_D(14)</div>
              <div>VWAP()</div>
              <div style={{color:C.am,marginTop:4}}>Bar data:</div>
              <div>open high low close volume</div>
              <div>prev_close prev_high prev_low</div>
              <div style={{color:C.pu,marginTop:4}}>Multi-Timeframe:</div>
              <div>HTF("W", w =&gt; w.SMA(20))</div>
              <div>HTF("M", m =&gt; m.RSI(14))</div>
              <div style={{color:C.cy,marginTop:4}}>Operators:</div>
              <div>&& (and) || (or) {">"} {"<"} {">="} {"<="}</div>
              <div style={{color:C.am,marginTop:4}}>Cross helpers:</div>
              <div>prev_SMA(p) prev_EMA(p) prev_RSI(p)</div>
            </div>
          </>:<>
          <div style={{marginBottom:14}}><div style={{fontSize:9,fontWeight:600,color:C.td,textTransform:"uppercase",letterSpacing:".1em",marginBottom:6}}>Presets</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:3}}>{Object.entries(PR).map(([k,v])=><Btn key={k} active={preset===k} onClick={()=>applyP(k)} style={{fontSize:9,padding:"3px 8px"}}>{v.n}</Btn>)}</div></div>

          <div style={{marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}><div style={{fontSize:9,fontWeight:600,color:C.g,textTransform:"uppercase",letterSpacing:".1em"}}>▲ Entry {enR.length>1?"(ALL)":""}</div><button onClick={()=>addR("en")} style={{background:C.gd,color:C.g,border:`1px solid ${C.g}30`,borderRadius:4,padding:"2px 7px",fontSize:9,fontFamily:MO,cursor:"pointer"}}>+</button></div>
          ...
          <div style={{marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}><div style={{fontSize:9,fontWeight:600,color:C.r,textTransform:"uppercase",letterSpacing:".1em"}}>▼ Exit {exR.length>1?"(ANY)":""}</div><button onClick={()=>addR("ex")} style={{background:C.rd,color:C.r,border:`1px solid ${C.r}30`,borderRadius:4,padding:"2px 7px",fontSize:9,fontFamily:MO,cursor:"pointer"}}>+</button></div>
          ...
          <div style={{marginBottom:14}}><div style={{fontSize:9,fontWeight:600,color:C.td,textTransform:"uppercase",letterSpacing:".1em",marginBottom:6}}>Config</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5}}>
            {[["Capital","cap",100,1e6],["Size %","sz",1,100],["Stop %","sl",0,50],["TP %","tp",0,100]].map(([l,k,mn,mx])=><div key={k}><div style={{fontSize:8,color:C.td,fontFamily:MO,marginBottom:2}}>{l}</div><Num value={cfg[k]} onChange={v=>setCfg(c=>({...c,[k]:v}))} min={mn} max={mx} step={k==="sl"||k==="tp"?.5:1} style={{width:"100%"}}/></div>)}
          </div>
          <div style={{marginTop:5}}><div style={{fontSize:8,color:C.td,fontFamily:MO,marginBottom:2}}>Direction</div><div style={{display:"flex",gap:3}}>{["long","short"].map(d=><Btn key={d} active={cfg.dir===d} color={d==="long"?C.g:C.r} onClick={()=>setCfg(c=>({...c,dir:d}))} style={{flex:1,fontSize:9,padding:"4px 0",textTransform:"uppercase"}}>{d}</Btn>)}</div></div></div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:16}}>
        ...
        {pg==="bt"&&<div className="fi">
          {dataLoading&&<div style={{background:C.s2,borderRadius:10,padding:20,textAlign:"center",marginBottom:14,fontFamily:MO,fontSize:12,color:C.am}}>⏳ Fetching {ASSETS.find(a=>a.id===asset)?.name} market data...</div>}
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{fontFamily:MO,fontSize:10,color:C.td}}>{ASSETS.find(a=>a.id===asset)?.sym}/USD · {data.length} candles · {data[0]?.date} → {data[data.length-1]?.date}</div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:7,marginBottom:14}}>
            <Stat label="Return" value={`${s.ret>0?"+":""}${s.ret}%`} sub={`$${s.fin.toLocaleString()}`} color={rc}/><Stat label="Win Rate" value={`${s.wr}%`} sub={`${s.wn}W/${s.ls}L`} color={s.wr>=50?C.g:C.am}/><Stat label="Profit Factor" value={s.pf>=999?"∞":s.pf} color={s.pf>=1.5?C.g:s.pf>=1?C.am:C.r}/><Stat label="Sharpe" value={s.sh} color={s.sh>=1?C.g:C.am}/><Stat label="Max DD" value={`${s.mdd}%`} color={C.r}/><Stat label="Expectancy" value={`$${s.exp}`} sub="per trade" color={s.exp>0?C.g:C.r}/><Stat label="Avg Hold" value={`${s.ah}d`} sub={`${s.n} trades`}/>
          </div>
          <div style={{display:"flex",gap:3,marginBottom:10}}>{[["eq","Equity"],["dd","Drawdown"],["pr","Price+Trades"],["di","P&L Dist"]].map(([id,l])=><Btn key={id} active={ct===id} onClick={()=>setCt(id)} style={{fontSize:10}}>{l}</Btn>)}</div>
          <div style={{background:C.sf,border:`1px solid ${C.bd}`,borderRadius:10,padding:14,marginBottom:12}}>
